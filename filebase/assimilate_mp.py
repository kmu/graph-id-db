...
def assimilate_json_files(input_dir: str, output_dir: str) -> None:
    ...
    # Find all JSON files
    json_files = list(input_dir_path.glob("id_json/*.json"))
    ...
    reduced_comp_str = str(structure.composition.reduced_composition).replace(' ', '')
    print(reduced_comp_str)

    reduced_comp_str = re.sub(r'(?<=\D)1(?=\D|$)', '', reduced_comp_str)
    dir_path = output_dir_path / reduced_comp_str
